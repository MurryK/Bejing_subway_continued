...
int City::linked(int id1, int id2)
{
	for (int i = 0; i < stas[id1]->getvnum(); i++) {
		for (int j = 0; j < stas[id2]->getvnum(); j++) {
			if (vlink[stas[id1]->getvid(i)][stas[id2]->getvid(j)] == 1) {
...
int *City::findway(char *s1, char *s2, int type)
{
	...
	int que[Max_Sta];
	int * way = (int* )malloc(Max_Sta*sizeof(int));
	id1 = str2id(s1);
	if (id1 == -1) {
		cout << s1 << "不存在" << endl;
		return NULL;
...
int City::sta2line(int id)
{
	int lineid;
	if (stas[id]->getvnum() == 1) {
		lineid = v2pandl[stas[id]->getvid(0)][1];
		return lineid;
	}
	return -1;
}
